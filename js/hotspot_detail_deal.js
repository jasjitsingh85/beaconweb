...
function make_redeem_inactive() {
    $('.redeem-button').attr('src',"http://s3-us-west-2.amazonaws.com/hotspot-static/static/img/redeemButtonInactive.png");
}

function make_redeem_active() {
    $('.redeem-button').attr('src',"http://s3-us-west-2.amazonaws.com/hotspot-static/static/img/redeemButtonActive.png");
}

function redeem_deal() {
    hotspot.redeemDeal(dealID ,function(success){
        make_redeem_inactive();
        alert(success.message);
        if (iOS) {
            setTimeout(function() {
                var confirm_return = confirm('Remember to tip!\n\nCheck out Hotspot on the App Store?');
                if (confirm_return== true) {
                    window.location = "https://itunes.apple.com/us/app/hotspot-get-your-friends-to/id737629048";
                }
            }, 3000);
        }
    });
...
function start_countdown() {
    hotspot.getDeal(dealID, function(success){
        var start = new Date((success.contact_deal_status.start_time) * 1000);
        var end = new Date((success.contact_deal_status.end_time) * 1000);
        var now = new Date();
        ...
        if (end < now) {
            ...
            deal_is_coming_up(start);
        }
    });
}

function deal_has_expired(){
    make_redeem_inactive();
    $('.time-body, .deal-info-body,.deal-info-header').html("");
    $('.time-heading').html("Deal has expired").addClass('deal-expired');
}

function deal_is_active(end) {
    $('.time-heading').html("Expires in:");
    init_countdown(end);
}

function deal_is_coming_up(start) {
    make_redeem_inactive();
    $('.time-heading').html("Begins in:");
    $('.time-heading, .time-body').css("color", "#ea6d5a")
    init_countdown(start);
}

function init_countdown(end) {
    $('.time-body').countdown({
          date: end,
          render: function(data) {
            $(this.el).text( data.hours + ":" + this.leadingZeros(data.min, 2) + ":" +this.leadingZeros(data.sec, 2));
          },
          onEnd: function() {
...
function switch_to_deals(self){
    self.find('img').attr("src","http://s3-us-west-2.amazonaws.com/hotspot-static/static/img/dealButtonSelected.png");
...
function switch_to_guest(self) {
	self.find('img').attr("src","http://s3-us-west-2.amazonaws.com/hotspot-static/static/img/chatButtonSelected@2x.png");
	self.find('img').transition({ scale: [1.25, 1.25], duration: 100 }).transition({ scale: [1, 1], delay: 25 });
	$('.guest-tab').find('img').attr("src","http://s3-us-west-2.amazonaws.com/hotspot-static/static/img/invitedButtonNormal@2x.png");
    $('.deal-tab').find('img').attr("src","http://s3-us-west-2.amazonaws.com/hotspot-static/static/img/dealButtonNormal.png");
	$('.deal-list').hide();
    $('.guest-list').hide();
	$('.chat-list').show();
    setTimeout(function(){
...
function select_deals(self) {
	if (self.hasClass('active')) {
		toggle_map_and_info();
	} else {
		switch_to_deals(self);
	}
	remove_active();
	add_active(self);
}
...
function remove_touch_events() {
	$('.chat-tab, .guest-tab, .deal-tab').hammer().off('touch');
	$('.chat-tab, .guest-tab, .deal-tab').hammer().off('release');
}
...
function add_event_listeners() {
    ...
    $('.deal-tab').hammer().on('touch', function() {
		var chat_image = $(this).find('img');
		selected(chat_image);
	});

	$('.deal-tab').hammer().on('release', function() {
		var self = $(this);
		unselected(self.find('img'));
		select_deals(self);
	});
...
//			$('.send-message-box').click();
    ...
    if (dealID != "None") {
        start_countdown();
        $('.redeem-button').hammer().on('touchstart', function(){
            var confirm_return = confirm('Only staff can redeem a deal. This cannot be undone\n\nAre you sure?');
            if (confirm_return== true) {
                redeem_deal();
            }
        });
